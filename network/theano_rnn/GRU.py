...
class GRU(object):
    def __init__(self,n_in,hidden,n_out,activation,output_type,error_type,learn_rate,moment,L1_reg,L2_reg):
        ...
        self.y = T.matrix('y')
        #gate params
        self.wz = theano.shared(np.asarray(np.random.uniform(low=-0.01,high=0.01,size=(hidden,n_in)),dtype=theano.config.floatX))
        self.uz = theano.shared(np.asarray(np.random.uniform(low=-0.01,high=0.01,size=(hidden,hidden)),dtype=theano.config.floatX))
        #reset params
        self.wr = theano.shared(np.asarray(np.random.uniform(low=-0.01,high=0.01,size=(hidden,n_in)),dtype=theano.config.floatX))
        self.ur = theano.shared(np.asarray(np.random.uniform(low=-0.01,high=0.01,size=(hidden,hidden)),dtype=theano.config.floatX))
        #
        ...
        self.b_z = theano.shared(np.zeros((hidden,),dtype=theano.config.floatX))
        self.b_r = theano.shared(np.zeros((hidden,),dtype=theano.config.floatX))
        self.b = theano.shared(np.zeros((hidden,),dtype=theano.config.floatX))
        self.params = [self.wz,self.uz,self.wr,self.ur,self.w,self.u,self.b_z,self.b_r,self.b,self.w_ho,self.b_o,self.h0]
        ...
        for param in self.params:
            self.updates[param] = theano.shared(value = np.zeros(
                                                ...
                                                name = 'updates')
        def step(u_t,h_tm1):
            h_t = self.activation_function(u_t, h_tm1)
#            c_t = h_t
...
#        self.L2 = ((self.w_ih) **2 ).sum() + ((self.w_hh) **2 ).sum() + ((self.w_ho) **2 ).sum()        
        ...
        if self.error_type == 'mse':
            ...
            self.cost = -T.mean(T.log(self.predicts)[T.arange(y.shape[0]), y])
                ...
                raise NotImplementedError 
    def activation_function(self,x_t,h_tm1):
        zt = T.nnet.sigmoid(T.dot(self.wz,x_t)+T.dot(self.uz,h_tm1)+self.b_z)
        rt = T.nnet.sigmoid(T.dot(self.wr,x_t)+T.dot(self.ur,h_tm1)+self.b_r)
        h_temp = T.tanh(T.dot(self.w,x_t)+T.dot(self.u,h_tm1) * rt + self.b)
        h_t = (1-zt) * h_tm1 + zt * h_temp
        return h_t
...
def example():
    ...
    myrnn = GRU(n_in = n_in, hidden = hidden, n_out=n_out,activation = 'tanh', output_type = 'real',error_type = 'mse',learn_rate = 0.001, L1_reg = 0, L2_reg = 0, moment = 0.9)
