...
class NN(object): 
	def __init__(self,architecture,activation_function='sigm',learningRate = 2,momentum = 0.5,weightPenaltyL2 = 0,sparsityparameter = 0.1,beta=0.5,inputzeroMaskedFraction=0,output = 'sigm',jacobi_penalty = 0,scaling_learningRate = 0.99,dropout = 0):
	...
	def nnff(self,x,y):
		...
		for i in range(2,n):
			...
			if self.activation_function == 'sigm':
				self.a[str(i)] = sigm(np.dot(self.a[str(i-1)],self.W[str(i-1)]))
			elif self.activation_function == 'tanh_opt':
				self.a[str(i)] = tanh_opt(np.dot(self.a[str(i-1)],self.W[str(i-1)]))
			if self.dropout > 0:
				if self.test == 0:
					self.a[str(i)][np.random.uniform(0,1,np.shape(self.a[str(i)])) < self.dropout] = 0
				else:
...
#			self.a[str(i)][:,1:] = x_a[str(i)]
		...
		if self.output == 'sigm':
			...
			self.a[str(n)] = np.dot(self.a[str(n-1)],self.W[str(n-1)])+self.b[str(n-1).mean(0)]
		elif self.output == 'softmax':
			self.a[str(n)] = softmax(np.dot(self.a[str(n-1)],self.W[str(n-1)])+self.b[str(n-1)].mean(0))		
...
#			if i+1 == n:
				self.dW[str(i)] = np.dot(self.a[str(i)].T,d[str(i+1)])/np.shape(d[str(i+1)])[0]
